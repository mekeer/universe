;(function (global, factory) {
  ...
  class initUniverseByRandom {
    constructor(
      ...
      minQuality = 100,
      maxQuality = 1000,
      minSpe = new Spe(-100, -100),
      maxSpe = new Spe(100, 100)
    ) {
      this.randomlyGeneratedArea = new Area(
        ...
        for (let i = 0; i < quadtTree.list.length; i++) {
          ...
          if (star.position.y + star.r < quadtTree.bigStar.position.y) {
            ...
            if (star.position.x + star.r < quadtTree.bigStar.position.x) {
              ...
              star.position.x - star.r >
              quadtTree.bigStar.position.x
            ) {
              //如果在y轴分隔线右边
              //也包括了圆切线到y轴的情况
              //上右
