(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global["@mekefly/universe"] = {}));
  ...
  class Star extends Render {
      ...
      checkIsPress(position) {
          let c = Position.distance(this.position, position);
          return c < this.r;
      }
  }
  ...
  class ShowScreen {
      ...
      constructor(canvas, ctx, list, position = new Position(), height, width, fps = 60) {
          ...
          this.drag = (event) => {
              if (this.accordingToStar) {
                  //求相对于上一次移动当前小球移动的速度
                  let absolutePositon = this.relativeToAbsolute(new Position(event.clientX, event.clientY));
      ...
      speedMov(movePositionBefore, moveTimeBefore, movePositionAfter, moveTimeAfter) {
          ...
          let xSpe = ((movePositionAfter.x - movePositionBefore.x) /
              beforeMovingToPresentTime) *
              1000;
          let ySpe = ((movePositionAfter.y - movePositionBefore.y) /
              beforeMovingToPresentTime) *
              1000;
  ...
  class Universe extends Render {
      ...
      showConstellation(ctx, showScreen) {
          this.ctx.save();
          this.ctx.beginPath();
          let newStartPos = this.absoluteToRelative(this.constellation.startPosition, showScreen);
          let newEndPos = this.absoluteToRelative(this.constellation.endPosition, showScreen);
      ...
      quadtreeComputingGravity() {
          ...
          const compute = (quadtTree, star) => {
              ...
              if (quadtTree.leftTop)
                  compute(quadtTree.leftTop, star);
              if (quadtTree.rightTop)
                  compute(quadtTree.rightTop, star);
              if (quadtTree.leftBottom)
                  compute(quadtTree.leftBottom, star);
              if (quadtTree.rightBottom)
                  compute(quadtTree.rightBottom, star);
              if (!quadtTree.leftTop &&
                  !quadtTree.rightTop &&
